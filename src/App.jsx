    ...
    try {
      ...
      if (invite) {
        const { data: newUser, error: createError } = await supabase.from('profiles').insert([{ 
            username: inputName, is_verified: true, last_mock_date: new Date().toISOString().split('T')[0] 
          }]).select().single();

        if (newUser && !createError) {
          await supabase.from('invite_codes').update({ is_used: true }).eq('id', invite.id);
          setUser(newUser);
        } else setLoginError("Error initializing node.");
      } else setLoginError("❌ Access Denied.");
    } catch (err) { console.error("Login Error", err); setLoginError("System Failure."); }
  };
    ...
    return (
      <div className={`flex items-center justify-center min-h-screen ${isDarkMode ? 'bg-slate-950' : 'bg-blue-50'}`}>
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] shadow-2xl border-2 border-blue-500/20 w-full max-w-md text-center animate-in fade-in zoom-in duration-500">
          ...
          <input className="w-full p-5 rounded-2xl border-2 mb-4 dark:bg-slate-800 dark:border-slate-700 dark:text-white font-black outline-none focus:border-blue-500 text-center" placeholder="USERNAME OR INVITE CODE" value={username} onChange={(e) => setUsername(e.target.value)} />

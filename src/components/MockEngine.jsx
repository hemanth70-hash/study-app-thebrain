...
export default function MockEngine({ user, onFinish }) {
        ...
        const handleVisibility = () => {
          if (document.hidden) {
            setWarnings(prev => {
              const next = prev + 1;
              if (next >= 3) {
                handleSubmit(true); // Forced 0% Penalty
                return next;
              }
              alert(`STRICT MODE WARNING: Tab switch detected. Strike ${next}/3. Third strike results in Disqualification.`);
              return next;
            });
          }
        };
  ...
  const handleSubmit = async (isPenalty = false) => {
    ...
    try {
      ...
      setIsFinished(true);
      loadMockData(); 
      if (isPenalty) alert("SECURITY BREACH: Disqualified. Score: 0%");
    } catch (err) {
      alert("Neural Grid Sync Error.");
    }
  };
            ...
            return (
              <button key={mock.id} disabled={mock.is_daily && isDone} onClick={() => startMock(mock)} 
                className={`p-8 rounded-[32px] text-left transition-all shadow-xl border-b-8 relative group ${
                  mock.is_daily ? (isDone ? 'bg-gray-100 dark:bg-gray-800 opacity-60' : 'bg-gradient-to-br from-orange-500 to-red-600 text-white') : 'bg-white dark:bg-gray-800 dark:text-white hover:scale-[1.02]'
                }`}>
      ...
      <div className="max-w-md mx-auto text-center p-12 bg-white dark:bg-gray-800 rounded-[40px] shadow-2xl border-t-8 border-green-500 relative overflow-hidden">
        {showStreakAnim && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-orange-600 z-50 animate-in fade-in zoom-in duration-500 text-white p-6 text-center">
            ...
            <h2 className="text-5xl font-black uppercase tracking-tighter">STREAK +1</h2>

...
import { supabase } from '../supabaseClient';
import { Timer, CheckCircle, Play, Layout, Zap, Award, Clock, ArrowLeft, Eye, Lock, ShieldAlert, AlertTriangle } from 'lucide-react';

export default function MockEngine({ user, onFinish }) {
  const [availableMocks, setAvailableMocks] = useState([]);
  const [selectedMock, setSelectedMock] = useState(null);
  const [questions, setQuestions] = useState([]); // Flat list for calculation
  const [subjects, setSubjects] = useState([]); // Grouped for CBT UI
  const [activeSubject, setActiveSubject] = useState(""); 
  const [currentIdx, setCurrentIdx] = useState(0); // Index within active subject
  const [selectedOptions, setSelectedOptions] = useState({});
  const [timeLeft, setTimeLeft] = useState(0); 
  const [isFinished, setIsFinished] = useState(false);
  const [loading, setLoading] = useState(true);
  ...
  const [completedMockIds, setCompletedMockIds] = useState([]); 
  const [showStreakAnim, setShowStreakAnim] = useState(false);
  const [warnings, setWarnings] = useState(0); // Proctoring strikes

  // --- 1. NEURAL TIMER & STRICT PROCTORING ---
  useEffect(() => {
    let interval = null;
    if (selectedMock && !isFinished && timeLeft > 0) {
      interval = setInterval(() => {
        ...
        const handleVisibility = () => {
          if (document.hidden) {
            setWarnings(prev => {
              const next = prev + 1;
              if (next >= 3) {
                alert("THE BRAIN: 3 Strikes Reached. Auto-Submitting.");
                handleSubmit();
              } else {
  ...
  const loadMockData = useCallback(async () => {
    ...
    try {
      ...
      if (mockData) setAvailableMocks(mockData.sort((a, b) => (b.is_daily ? 1 : -1)));
    } catch (err) {
      console.error("The Brain - Sync Error:", err);
    } finally {
      setLoading(false);
  ...
  // Maps local subject question ID to absolute flat index for scoring
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {availableMocks.map((mock) => {
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className={`p-3 rounded-2xl ${mock.is_daily ? (isDone ? 'bg-gray-200' : 'bg-white/20') : 'bg-blue-50 dark:bg-gray-700 text-blue-600'}`}>
                    {isDone && mock.is_daily ? <Lock size={24} /> : (mock.is_daily ? <Zap size={24} /> : <Layout size={24} />)}
                  </div>
                ...
                <h4 className="text-xl font-black uppercase mb-1 tracking-tight">{mock.mock_title}</h4>
                <p className="text-[10px] opacity-70 font-black uppercase tracking-widest italic">
                  {isDone && mock.is_daily ? 'LOCKED • STREAK SECURED' : (mock.is_strict ? 'STRICT CBT MODE' : (mock.is_daily ? 'STREAK ENABLED' : 'PRACTICE MODE'))}
                </p>
              </button>
            );
          })}
  ...
  if (isFinished) {
    ...
    if (showReview) {
      return (
        <div className="space-y-6 max-w-3xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <button onClick={() => setShowReview(false)} className="bg-white dark:bg-gray-800 px-6 py-3 rounded-2xl shadow-sm border dark:border-gray-700 font-black uppercase text-xs flex items-center gap-2 dark:text-white">
             <ArrowLeft size={16} /> Back to Results
          </button>
          {questions.map((q, idx) => (
            <div key={idx} className={`p-8 rounded-[2.5rem] border-l-8 bg-white dark:bg-gray-800 shadow-xl ${selectedOptions[idx] === q.correct_option ? 'border-green-500' : 'border-red-500'}`}>
      ...
      <div className="max-w-md mx-auto text-center p-12 bg-white dark:bg-gray-800 rounded-[40px] shadow-2xl border-t-8 border-green-500 relative overflow-hidden">
        {showStreakAnim && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-orange-600 z-50 animate-in fade-in zoom-in duration-500 text-white p-6 text-center">
            ...
            <h2 className="text-5xl font-black uppercase italic tracking-tighter mb-2">STREAK +1</h2>
            <button onClick={() => setShowStreakAnim(false)} className="mt-10 bg-white text-orange-600 px-12 py-4 rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-2xl active:scale-95 transition-transform hover:scale-110">Continue</button>
          </div>
        )}
        <Award size={64} className="mx-auto text-green-500 mb-6" />
        <h2 className="text-3xl font-black dark:text-white uppercase tracking-tighter">Mock Complete!</h2>
        ...
        <div className="flex gap-2">
          {subjects.map(s => (
            <button key={s.subject} onClick={() => {setActiveSubject(s.subject); setCurrentIdx(0);}} 
              className={`px-6 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeSubject === s.subject ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 dark:bg-gray-900 text-gray-400'}`}>
              {s.subject}
            </button>
          ))}
        </div>
        <div className={`flex items-center gap-3 font-mono text-xl font-bold px-6 py-2 rounded-xl ${timeLeft < 60 ? 'bg-red-500 text-white animate-pulse' : 'bg-black text-white'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border dark:border-gray-700 relative">
          {selectedMock.is_strict && (
            <div className="absolute top-6 right-10 flex items-center gap-2 text-red-500 font-black text-[10px] uppercase">
              <ShieldAlert size={14} /> Strikes: {warnings}/3
            </div>
          )}
          <h4 className="text-blue-600 font-black uppercase text-[10px] tracking-widest mb-4">{activeSubject} / Question {currentIdx + 1}</h4>
            ...
            {activeSubData.questions[currentIdx].options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => setSelectedOptions({...selectedOptions, [absIdx]: i})} 
                  className={`w-full text-left p-6 rounded-2xl border-2 transition-all font-bold flex items-center gap-4 ${selectedOptions[absIdx] === i ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 dark:text-white' : 'border-gray-100 dark:border-gray-700 dark:text-gray-300'}`}>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${selectedOptions[absIdx] === i ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700'}`}>{String.fromCharCode(65 + i)}</div>
                  {opt}
                </button>
              )
            })}
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[3rem] shadow-xl border dark:border-gray-700">
          <p className="text-[10px] font-black uppercase text-gray-400 mb-6 tracking-widest">Questions</p>
            ...
            {activeSubData.questions.map((_, i) => {
              ...
              return (
                <button key={i} onClick={() => setCurrentIdx(i)} 
                  className={`aspect-square rounded-xl font-black text-xs transition-all ${currentIdx === i ? 'ring-2 ring-blue-500 ring-offset-2' : ''} ${isDone ? 'bg-green-500 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-900 text-gray-400'}`}>
                  {i + 1}
                </button>
              )
            })}
          </div>
          <button onClick={handleSubmit} className="w-full bg-red-600 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all">End Test</button>
        </div>
      </div>
    </div>
  );
...
import { 
  ...
  Clock, ArrowLeft, Eye, Lock, ShieldAlert, AlertTriangle 
} from 'lucide-react';
...
export default function MockEngine({ user, onFinish }) {
        ...
        const handleVisibility = () => {
          if (document.hidden) {
            setWarnings(prev => {
              const next = prev + 1;
              if (next >= 3) {
                handleSubmit(true); // Auto-submit with 0% Penalty
                return next;
              }
              alert(`STRICT MODE WARNING: Tab switch detected. Strike ${next}/3. Third strike results in 0%.`);
              return next;
            });
          }
        };
  ...
  const handleSubmit = async (isPenalty = false) => {
    ...
    try {
      ...
      setIsFinished(true);
      loadMockData(); 
      if (isPenalty) alert("SECURITY BREACH: Test Terminated. Score: 0%");
    } catch (err) {
          ...
          {availableMocks.map((mock) => {
            ...
            return (
              <button key={mock.id} disabled={mock.is_daily && isDone} onClick={() => startMock(mock)} 
                className={`p-8 rounded-[32px] text-left transition-all shadow-xl border-b-8 relative group ${
                  mock.is_daily ? (isDone ? 'bg-gray-100 dark:bg-gray-800/50 text-gray-400 border-gray-200' : 'bg-gradient-to-br from-orange-500 to-red-600 text-white') : 'bg-white dark:bg-gray-800 dark:text-white hover:scale-[1.02]'
                }`}>
                <div className="flex justify-between items-start mb-4">
                ...
                <h4 className="text-xl font-black uppercase mb-1 tracking-tight">{mock.mock_title}</h4>
                <p className="text-[10px] opacity-70 font-black uppercase tracking-widest italic">{isDone ? 'STREAK SECURED' : (mock.is_strict ? 'STRICT MODE' : 'PRACTICE MODE')}</p>
              </button>
            );
          })}
        </div>
  ...
  if (isFinished) {
    ...
    if (showReview) {
      return (
        <div className="space-y-6 max-w-3xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <button onClick={() => setShowReview(false)} className="bg-white dark:bg-gray-800 px-6 py-3 rounded-2xl shadow-sm border dark:border-gray-700 font-black uppercase text-xs flex items-center gap-2 dark:text-white"><ArrowLeft size={16} /> Back to Results</button>
          {questions.map((q, idx) => (
            <div key={idx} className={`p-8 rounded-[2.5rem] border-l-8 bg-white dark:bg-gray-800 shadow-xl ${selectedOptions[idx] === q.correct_option ? 'border-green-500' : 'border-red-500'}`}>
              <p className="font-bold text-lg dark:text-white mb-4 leading-tight">{idx + 1}. {q.question}</p>
              <div className="grid grid-cols-1 gap-3">
    ...
    return (
      <div className="max-w-md mx-auto text-center p-12 bg-white dark:bg-gray-800 rounded-[40px] shadow-2xl border-t-8 border-green-500 relative overflow-hidden">
        {showStreakAnim && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-orange-600 z-50 animate-in fade-in zoom-in duration-500 text-white p-6 text-center">
            <div className="text-8xl animate-bounce mb-4">🔥</div>
            <h2 className="text-5xl font-black uppercase italic tracking-tighter">STREAK +1</h2>
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="bg-white dark:bg-gray-800 p-4 rounded-[2rem] shadow-xl flex flex-wrap justify-between items-center gap-4 border dark:border-gray-700">
        <div className="flex gap-2">
          {subjects.map(s => (
            <button key={s.subject} onClick={() => {setActiveSubject(s.subject); setCurrentIdx(0);}} 
              className={`px-6 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeSubject === s.subject ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 dark:bg-gray-900 text-gray-400'}`}>{s.subject}</button>
          ))}
        </div>
        <div className={`flex items-center gap-3 font-mono text-xl font-bold px-6 py-2 rounded-xl ${timeLeft < 60 ? 'bg-red-500 text-white animate-pulse' : 'bg-black text-white'}`}><Timer size={18} /> {formatTime(timeLeft)}</div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border dark:border-gray-700 relative">
          ...
          <h4 className="text-blue-600 font-black uppercase text-[10px] tracking-widest mb-4 italic">{activeSubject} / Q{currentIdx + 1}</h4>
          <h3 className="text-2xl font-bold dark:text-white mb-10 leading-tight">{activeSubData.questions[currentIdx].question}</h3>
          <div className="grid grid-cols-1 gap-4">
            {activeSubData.questions[currentIdx].options.map((opt, i) => {
              const absIdx = getAbsIdx(activeSubject, currentIdx);

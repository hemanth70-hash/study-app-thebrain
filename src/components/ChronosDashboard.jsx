...
import { 
  ...
  Flame, Download, FileText, X, AlertCircle, ArrowUpRight, Radio, ExternalLink, Globe
} from 'lucide-react';
...
export default function ChronosDashboard({ user }) {
  ...
  const [hybridFeed, setHybridFeed] = useState([]); // 🔥 LIVE FEED STATE

  // --- 1. PERSISTENCE & HYBRID FEED ENGINE ---
  useEffect(() => {
    // A. Local Storage Load
    ...
    // B. HYBRID FEED FETCH (Internal + External)
              ...
              {hybridFeed.map((item) => (
                 <div key={item.id} className={`p-3 rounded-xl border-l-2 bg-slate-900/40 hover:bg-slate-800/60 transition-colors ${item.source === 'WEB' ? 'border-blue-500' : item.type === 'win' ? 'border-green-500' : item.type === 'streak' ? 'border-orange-500' : 'border-purple-500'}`}>
                    <div className="flex justify-between items-start mb-1">
                       <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded text-white flex items-center gap-1 ${item.source === 'WEB' ? 'bg-blue-600' : item.type === 'win' ? 'bg-green-600' : item.type === 'streak' ? 'bg-orange-600' : 'bg-purple-600'}`}>
                          {item.source === 'WEB' ? <Globe size={8} /> : null} {item.source === 'WEB' ? 'INTERNET' : item.type.toUpperCase()}
                       </span>
        ...
        <div className="lg:col-span-6 bg-[#0a0a0f] border border-slate-800 rounded-[2rem] p-8 shadow-xl">
           <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2 text-[9px] font-bold uppercase text-slate-500">
                 <div className="flex items-center gap-1"><div className="w-2 h-2 rounded bg-slate-900 border border-slate-700"></div> Future</div>
           ...
           <div className="grid grid-cols-7 gap-3">
              ...
              {calendarDays.map(day => {
                 ...
                 return <div key={day} className={`aspect-square rounded-lg flex items-center justify-center text-xs font-bold border transition-all ${isFuture ? 'border-transparent text-slate-700 bg-[#050508] cursor-not-allowed' : isToday ? 'border-white text-white bg-slate-800 shadow-lg scale-110' : 'border-transparent text-slate-500 bg-slate-900'}`}>{day}</div>
              })}
           </div>
        </div>

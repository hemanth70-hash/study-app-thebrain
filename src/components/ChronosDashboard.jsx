...
import { 
  ...
  Flame, Download, FileText, X, AlertCircle, ArrowUpRight, Radio, ExternalLink, Globe, Loader2, Clock, Flag
} from 'lucide-react';
...
export default function ChronosDashboard({ user }) {
  ...
  // 🧠 ENHANCED GOAL STATE
  // Goals now have: { id, title, type: 'core'|'ops', date: 'YYYY-MM-DD', priority: 'high'|'normal', completed: boolean }
  ...
  useEffect(() => {
    ...
    // Live Feed Logic (Kept from previous version)
  ...
  const addGoal = () => { 
    ...
    const newGoal = { 
      id: Date.now(), 
      title: newGoalTitle, 
      type: newGoalType, // 'core' or 'ops'
      date: newGoalType === 'ops' ? newGoalDate : null, 
  ...
  return (
    <div className="w-full min-h-screen p-4 md:p-8 font-sans text-slate-100 animate-in fade-in zoom-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-7xl mx-auto">
        ...
        {/* BLOCK 1: BALLISTICS (Unchanged Logic) */}
        ...
        {/* BLOCK 2: CACHED HYBRID FEED (Unchanged Logic) */}
        <div className="lg:col-span-4 bg-[#0a0a0f] border border-slate-800 rounded-[2rem] p-6 shadow-xl flex flex-col h-[420px] overflow-hidden relative">
           <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-800 bg-[#0a0a0f] z-10">
              ...
              <span className="text-[9px] text-slate-600 font-mono flex items-center gap-1">{isFeedUpdating && <Loader2 size={8} className="animate-spin" />}{isFeedUpdating ? "SYNCING..." : "LIVE UPLINK"}</span>
           </div>
           <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3">
              {hybridFeed.map((item) => (
                 <div key={item.id} className={`p-3 rounded-xl border-l-2 bg-slate-900/40 hover:bg-slate-800/60 transition-colors ${item.source === 'WEB' ? 'border-blue-500' : item.type === 'win' ? 'border-green-500' : 'border-purple-500'}`}>
                    <div className="flex justify-between items-start mb-1"><span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded text-white flex items-center gap-1 ${item.source === 'WEB' ? 'bg-blue-600' : item.type === 'win' ? 'bg-green-600' : 'bg-purple-600'}`}>{item.source === 'WEB' ? <Globe size={8} /> : null} {item.source === 'WEB' ? 'INTERNET' : item.type.toUpperCase()}</span><span className="text-[9px] text-slate-600 font-mono">{new Date(item.time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span></div>
        ...
        <div className="lg:col-span-6 bg-[#0a0a0f] border border-slate-800 rounded-[2rem] p-8 shadow-xl">
           ...
           <div className="grid grid-cols-7 gap-3">
              ...
              {calendarDays.map(day => {
                 ...
                 // Check if ANY Dated goal exists for this specific day
                 ...
                 let style = "";
                 if (isFuture) style = "border-transparent text-slate-800 bg-[#050508] cursor-not-allowed";
                 else if (isToday) style = "border-white text-white bg-slate-800 shadow-lg scale-110"; 
                 else style = "border-transparent text-slate-500 bg-slate-900";
        ...
        {/* =======================================================
            BLOCK 4: TACTICAL OBJECTIVES SYSTEM
        ======================================================= */}
        <div className="lg:col-span-6 bg-[#0a0a0f] border border-slate-800 rounded-[2rem] p-8 shadow-xl flex flex-col">
           <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex bg-slate-900 rounded-lg p-1">
                 <button onClick={() => setNewGoalType('ops')} className={`px-3 py-1 rounded text-[10px] font-bold uppercase transition-all ${newGoalType === 'ops' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}>OPS (Dated)</button>
                 <button onClick={() => setNewGoalType('core')} className={`px-3 py-1 rounded text-[10px] font-bold uppercase transition-all ${newGoalType === 'core' ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500'}`}>CORE (Main)</button>
              </div>
           </div>
           ...
           <div className="flex flex-col gap-2 mb-6 p-4 bg-slate-900/50 rounded-xl border border-slate-700">
              <div className="flex gap-2">
                 <input 
                   ...
                   placeholder={newGoalType === 'core' ? "Set Strategic Directive..." : "Set Tactical Mission..."} 
                   className="flex-1 bg-transparent text-sm text-white focus:outline-none placeholder:text-slate-600 font-medium" 
                 />
                 <button onClick={addGoal} className="bg-white text-black p-1.5 rounded hover:bg-slate-200"><Plus size={16} /></button>
              </div>
              
              {/* Detailed Controls for OPS */}
              {newGoalType === 'ops' && (
                <div className="flex gap-3 pt-2 border-t border-slate-800">
                   <input type="date" value={newGoalDate} onChange={(e) => setNewGoalDate(e.target.value)} className="bg-slate-800 text-[10px] text-slate-300 rounded px-2 py-1 outline-none border border-slate-700" />
                   <select value={newGoalPriority} onChange={(e) => setNewGoalPriority(e.target.value)} className="bg-slate-800 text-[10px] text-slate-300 rounded px-2 py-1 outline-none border border-slate-700">
           ...
           <div className="flex-1 space-y-2 overflow-y-auto custom-scrollbar pr-1 max-h-[300px]">
              ...
              {/* SECTION: OPS (DATED) */}
              {goals.filter(g => g.type === 'ops' && !g.completed).length > 0 && (
                 <div className="mb-4">
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-2 px-1">Active Timeline</p>
                    <div className="space-y-2">
                       {goals.filter(g => g.type === 'ops' && !g.completed).sort((a,b) => new Date(a.date) - new Date(b.date)).map(goal => {
                          const timer = getTimeRemaining(goal.date);
                          return (
                             <div key={goal.id} className={`flex items-center gap-3 p-3 rounded-xl border bg-slate-900/40 cursor-pointer group hover:border-slate-600 transition-all ${goal.priority === 'high' ? 'border-red-900/50' : 'border-slate-800'}`} onClick={() => toggleGoal(goal.id)}>
                                <div className={`w-4 h-4 rounded border flex items-center justify-center border-slate-600 group-hover:border-blue-500 transition-colors`}></div>
                                <div className="flex-1">
                                   <div className="flex justify-between items-start">
                                      <span className="text-sm text-slate-200 font-medium">{goal.title}</span>
                                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded ml-2 ${timer.color} ${timer.bg}`}>{timer.text}</span>
                                   </div>
                                   <p className="text-[10px] text-slate-500 font-mono flex items-center gap-1"><Clock size={8} /> {goal.date}</p>
                                </div>
                                <button onClick={(e) => deleteGoal(goal.id, e)} className="opacity-0 group-hover:opacity-100 text-slate-600 hover:text-red-500 transition-opacity"><X size={14}/></button>
                             </div>
                          )
                       })}
                    </div>
              ...
              {/* SECTION: CORE (MAIN) */}
              <div className="mb-2">
                 <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-2 px-1">Strategic Directives</p>
                 {goals.filter(g => g.type === 'core' && !g.completed).map(goal => (
                    <div key={goal.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-800 bg-slate-900/20 cursor-pointer group hover:border-purple-500/30 transition-all mb-2" onClick={() => toggleGoal(goal.id)}>
                       <div className="w-4 h-4 rounded-full border border-slate-600 group-hover:border-purple-500 transition-colors"></div>
                       <span className="text-sm text-slate-300 flex-1">{goal.title}</span>
                       <button onClick={(e) => deleteGoal(goal.id, e)} className="opacity-0 group-hover:opacity-100 text-slate-600 hover:text-red-500 transition-opacity"><X size={14}/></button>
                    </div>
                 ))}
                 {goals.filter(g => g.type === 'core' && !g.completed).length === 0 && <div className="text-center text-xs text-slate-700 italic py-2">No strategic directives set.</div>}
              </div>

              {/* COMPLETED SECTION (Collapsed) */}
              {goals.some(g => g.completed) && (
                 <div className="mt-6 pt-4 border-t border-slate-800/50">
                    <p className="text-[9px] text-slate-600 font-bold uppercase tracking-widest mb-2">Completed Logs</p>
                    {goals.filter(g => g.completed).map(goal => (
                       <div key={goal.id} className="flex items-center gap-3 p-2 opacity-50 hover:opacity-100 transition-opacity cursor-pointer" onClick={() => toggleGoal(goal.id)}>
                          <CheckCircle size={14} className="text-green-500" />
                          <span className="text-xs text-slate-500 line-through decoration-slate-700">{goal.title}</span>
                       </div>
                    ))}
                 </div>
              )}

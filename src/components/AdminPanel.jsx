...
import { 
  ...
  GraduationCap, ChevronDown, BookOpen
} from 'lucide-react';
...
export default function AdminPanel() {
  ...
  // 🔥 REFINED UPLOAD: Fixes 409 Conflicts by clearing dependencies first
  const handleBulkUpload = async () => {
    if (!mockTitle || !bulkData) return setStatus('❌ Fields Required.');
    setStatus('⏳ Validating & Publishing...');
    try {
      const parsedQuestions = JSON.parse(bulkData);
      
      if (isDailyQuickMock) {
        // Find existing daily mock to prevent unique constraint conflict
        ...
        if (oldMocks && oldMocks.length > 0) {
          ...
          // Delete scores and completions first to satisfy Foreign Key constraints
      ...
      const { error } = await supabase.from('daily_mocks').insert([{ 
        ...
        is_strict: isStrict, // 🔥 Correctly passing the Strict Mode state
  ...
  const uploadResource = async () => {
    ...
    if (!error) { setBookTitle(''); setBookUrl(''); alert("Knowledge Synced to Library."); }
  };
  ...
  return (
    <div className="space-y-10 pb-20 max-w-7xl mx-auto p-4 animate-in fade-in duration-700">
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700 transition-all">
        ...
        <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest mt-1">{allUsers.length} Nodes Active</p>
        {showRoster && (
                  ...
                  return (
                    <tr key={u.id} className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl group transition-all hover:scale-[1.01]">
                      ...
                      <td className="px-6 py-4 text-right rounded-r-2xl">
                        ...
                        <p className="text-[8px] font-black text-gray-400 uppercase mt-1">Life: {u.total_exams_completed || 0}</p>
                      </td>
                    </tr>
                  );
                })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700">
          ...
          <div className="space-y-4">
            <input type="text" placeholder="Mock Title" className="w-full p-4 rounded-2xl border dark:bg-gray-900 dark:text-white font-bold outline-none focus:ring-2 focus:ring-blue-500" value={mockTitle} onChange={(e) => setMockTitle(e.target.value)} />
            <div className="grid grid-cols-2 gap-4">
              {/* Corrected Toggles */}
              <button type="button" onClick={() => setIsDailyQuickMock(!isDailyQuickMock)} className={`p-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 transition-all ${isDailyQuickMock ? 'bg-orange-500 text-white shadow-lg' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}><Zap size={14} /> Daily Mock</button>
              <button type="button" onClick={() => setIsStrict(!isStrict)} className={`p-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 transition-all ${isStrict ? 'bg-red-600 text-white shadow-lg' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}><ShieldAlert size={14} /> Strict Mode</button>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700">
          ...
          <div className="space-y-4">
            <input className="w-full p-4 rounded-2xl border dark:bg-gray-900 dark:text-white outline-none" placeholder="Resource Title (e.g. NCERT Physics)" value={bookTitle} onChange={e => setBookTitle(e.target.value)} />
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700">
          ...
          <div className="flex gap-2 mb-6">
            ...
            <button type="button" onClick={generateKey} className="bg-indigo-600 text-white p-4 rounded-2xl hover:bg-indigo-700"><UserPlus size={24} /></button>
          </div>
          <div className="space-y-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
            {activeKeys.map(k => (
              <div key={k.id} className="p-4 bg-gray-50 dark:bg-gray-900 rounded-2xl border dark:border-gray-800 flex justify-between items-center group transition-all">
                <div><p className="font-black text-indigo-600 text-xs tracking-widest">{k.access_key}</p><p className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Node: {k.assigned_to}</p></div>
                <button type="button" onClick={async () => { await supabase.from('authorized_users').delete().eq('id', k.id); fetchAdminData(); }} className="text-gray-300 hover:text-red-500 transition-all"><Trash2 size={16} /></button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-gray-900 text-white p-8 rounded-[32px] shadow-2xl border border-white/5 lg:col-span-1">
          ...
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
            {userRequests.length === 0 ? <p className="text-gray-500 text-center font-bold py-10 uppercase opacity-50 text-xs tracking-widest">No Signals...</p> : userRequests.map(req => (
              <div key={req.id} className="p-4 bg-white/5 rounded-2xl border border-white/10 hover:border-orange-500/50 transition-all relative group">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={async () => { await supabase.from('admin_requests').delete().eq('id', req.id); fetchAdminData(); }} className="flex-1 bg-green-600/20 text-green-400 py-2 rounded-xl font-black text-[10px] hover:bg-green-600/40 transition-all">RESOLVE</button>

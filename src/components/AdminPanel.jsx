...
export default function AdminPanel() {
  ...
  // --- 3. STUDY HUB RESOURCE STATE (LIBRARY) ---
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      if (isDailyQuickMock) {
        const { data: old } = await supabase.from('daily_mocks').select('id').eq('is_daily', true);
        if (old?.length > 0) {
      ...
      await supabase.from('daily_mocks').insert([{ 
        mock_title: mockTitle, questions: parsedQuestions, is_daily: isDailyQuickMock, 
        is_strict: isStrict, time_limit: parseInt(timeLimit), mock_date: new Date().toISOString().split('T')[0] 
      }]);
      setStatus(`🎉 Published!`);
  ...
  const uploadResource = async () => {
    ...
    const { error } = await supabase.from('study_resources').insert([{
      title: bookTitle, file_url: bookUrl, category: bookCategory
    }]);
  ...
  if (loading && allUsers.length === 0) return <div className="p-20 text-center animate-pulse font-black text-blue-600 uppercase">Connecting to Admin Grid...</div>;
  ...
  return (
    <div className="space-y-10 pb-20 max-w-7xl mx-auto p-4 animate-in fade-in duration-700">
      ...
      {/* 2. NEURAL ROSTER (THE BRAIN'S VIEW) */}
      <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700 transition-all">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowRoster(!showRoster)} className={`p-3 rounded-2xl bg-blue-50 dark:bg-gray-700 text-blue-600 transition-all duration-300 hover:scale-110 ${showRoster ? 'rotate-180 bg-blue-600 text-white shadow-lg' : ''}`}><ChevronDown size={28} strokeWidth={3} /></button>
        </div>
        <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest mt-1">{allUsers.length} Nodes Synchronized</p>
        {showRoster && (
                ...
                {allUsers.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl group transition-all hover:scale-[1.01]">
                      ...
                      <td className="px-6 py-4 text-center"><p className="font-black text-blue-600 text-lg">{(u.total_percentage_points / (u.total_exams_completed || 1)).toFixed(1)}%</p><p className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">Aggregate</p></td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleBulkUpload} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest shadow-xl active:scale-95">Publish Mock</button>
            {status && <p className="text-center font-black uppercase text-xs text-green-500 mt-2">{status}</p>}
          </div>
        </div>

        {/* 4. LIBRARY MANAGER (FOR STUDY HUB) */}
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700">
          ...
          <div className="space-y-4">
            ...
            <button onClick={uploadResource} className="w-full bg-orange-500 text-white py-4 rounded-2xl font-black uppercase shadow-lg active:scale-95 transition-all">Upload Resource</button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700">
          ...
          <div className="flex gap-2 mb-6">
            <input type="text" placeholder="Name" className="flex-1 p-4 rounded-2xl border dark:bg-gray-900 dark:text-white outline-none" value={assignedName} onChange={(e) => setAssignedName(e.target.value)} />
          ...
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
            {userRequests.length === 0 ? <p className="text-gray-500 text-center py-10 font-bold uppercase opacity-50 text-xs">No signals...</p> : userRequests.map(req => (
              <div key={req.id} className="p-4 bg-white/5 rounded-2xl border border-white/10 hover:border-orange-500/50 transition-all">
                ...
                <p className="text-xs font-black text-gray-300 uppercase">{req.user_name}</p>
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border dark:border-gray-700">
           ...
           <div className="space-y-3 mb-6">
            ...
            <button onClick={saveLog} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-lg hover:bg-indigo-700 active:scale-95">Commit Log</button>
          </div>

...
// =======================================================
// 1. GEMINI CONFIGURATION (FREE & FAST)
// =======================================================
...
const FISH = [
  { 
    ...
    sprite: '🐙', 
    type: 'mentor',
    systemPrompt: "You are a genius Octopus with 8 brains. Give multitasking study advice. Keep it under 15 words.",
    y: 40, speed: 0.3
  },
  { 
    id: 'crab', 
    ...
    sprite: '🦀', 
    type: 'greedy',
    systemPrompt: "You love efficiency. Tell the user time is money! Work harder! Keep it under 15 words.",
    y: 85, speed: 0.2
  },
  { 
    id: 'turtle', 
    ...
    sprite: '🐢', 
    type: 'chill',
    systemPrompt: "You are a chill sea turtle. Tell the user to ride the wave of knowledge. Keep it under 15 words.",
    y: 60, speed: 0.1
  },
  { 
    id: 'puffer', 
    ...
    sprite: '🐡', 
    type: 'nervous',
    systemPrompt: "You are a nervous Pufferfish. Panic about deadlines! Keep it under 15 words.",
    y: 20, speed: 0.4
  }
];
...
export default function PixelAquarium({ dailyScore, gpa, streak }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setSwimmers(prev => prev.map(s => {
        ...
        if (x > 90) dir = -1;
        if (x < 5) dir = 1;
        let y = s.y + Math.sin(Date.now() / 1000) * 0.1;
        return { ...s, x, dir, y };
      }));
    }, 50);
  ...
  const callGemini = async (prompt) => {
    ...
    try {
      ...
      // Safe check for valid response
      if (!data.candidates || data.error) return "My brain is tired (API Error)";
      return data.candidates[0].content.parts[0].text;
    } catch (e) {
      return "Connection lost to the ocean.";
    }
  };
  ...
  useEffect(() => {
    const loop = setInterval(async () => {
      ...
      if (Math.random() > 0.80) { 
        ...
        const f1 = swimmers[0]; // Octopus
        const f2 = swimmers[1]; // Crab

        const scriptPrompt = `Write a very short 2-line dialogue between a wise Octopus and a greedy Crab. Format: "Octopus: [text] | Crab: [text]"`;
  ...
  return (
    <div className="relative w-full h-64 bg-blue-900 rounded-xl overflow-hidden border-4 border-slate-800 mt-6 select-none group shadow-[0_0_30px_rgba(0,100,255,0.2)] font-sans">
      ...
      <div className="absolute bottom-0 w-full h-12 bg-gradient-to-t from-[#e6d5ac] to-[#c2b280] border-t-4 border-[#a69260] z-10 flex items-center justify-around">
         {[...Array(20)].map((_, i) => (
            <div key={i} className="w-1 h-1 bg-yellow-900/20 rounded-full" style={{ marginTop: Math.random() * 20 }}></div>
         ))}
      </div>
      ...
      {/* 4. BUBBLES */}
      {bubbles.map(b => (
        <div key={b.id} className="absolute rounded-full border border-white/40 bg-white/10 backdrop-blur-sm z-0" 
      ...
      {swimmers.map(s => (
        ...
        >
           <AnimatePresence>
             {dialogues[s.id] && (
               <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} 
                           className="absolute -top-16 w-32 bg-white text-blue-900 text-[10px] font-bold p-2 rounded-xl text-center z-50 border-2 border-blue-200 shadow-lg">
                 {dialogues[s.id]}
           ...
           <div className={`text-5xl filter drop-shadow-xl ${s.dir === -1 ? 'scale-x-[-1]' : ''}`}>
             {s.sprite}
           </div>
        </motion.div>
      ))}
        ...
        {chatTarget && (
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="absolute inset-0 bg-blue-950/95 z-50 flex flex-col p-4 backdrop-blur-md">
             <div className="flex justify-between items-center border-b border-white/20 pb-2 mb-2">
                <div className="flex items-center gap-2">
                   ...
                   <div>
                     ...
                     <div className="text-cyan-400 text-[9px] uppercase tracking-widest flex items-center gap-1">
                        <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse"></span> AquaLink
                     </div>
                   </div>
                </div>
             ...
             <div className="flex gap-2">
                <input autoFocus value={userVal} onChange={(e) => setUserVal(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} className="flex-1 bg-slate-900 border border-cyan-900 rounded-lg px-3 py-2 text-xs text-white outline-none" placeholder="Ask for wisdom..." />
